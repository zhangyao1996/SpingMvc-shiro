...
function permissionBtn(id) {
     ...
     */
    var setting = {
        view: { //view中包含了Tree树的一些视图样式配置，例如是否显示节点间的连线，是否显示节点的图标，等
            selectedMulti: false //selectedMulti是view的一个配置参数，设置是否允许同时选中多个节点。  
        },
        check: {
            enable: true
        },
        data: {//data中包含了要展示的数据以及展示数据的配置，因为我们采用了ajax请求数据，这里需要配置simpleData
            simpleData: {//simpleData数据展示的配置：enable是否采用简单的数据模式；idKey树节点ID名称；pIdKey父节点ID名称；rootPId根节点ID
    ...
    $(function () {
        ...
        $.ajax({
            ...
            success: (data) => {
                console.log(data);
                $.fn.zTree.init($("#tree"), setting, data);//初始化树节点时，添加同步获取的数据,setting是上面写的ztree的所有配置
                checkNodes();
            },
            error: () => {
                alert('初始化树节点失败！！！');
    ...
    //处理哪些节点应该被选中
    function checkNodes() {
        var zTree = $.fn.zTree.getZTreeObj("tree");//获取zTree对象，根据div中指定的ID获取此渲染的ZTree对象，下面的方法都用到此对象调用
